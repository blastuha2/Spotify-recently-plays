...
Из общего списка экспортируются прослушанные элементы типа `TRACK`; пустые элементы, альбомы, плейлисты и действия вроде `saved` не считаются прослушиваниями. Повторные появления одного URI сохраняются как отдельные события, поскольку они могут соответствовать повторным прослушиваниям.

Spotify группирует родительский источник и дочерние треки через `children_group_id` / `group_id_*`. Приложение использует эту связь и добавляет к каждой записи:

```text
source_type     playlist, album или другой тип источника
source_name     название источника, например машина
source_uri      spotify:playlist:...
activity_type   played
```

Благодаря этому при анализе можно исключить строки, у которых `source_type=playlist` и `source_name=машина`. Если Spotify не указал источник для группы, эти поля остаются пустыми; приложение не выдумывает контекст.
